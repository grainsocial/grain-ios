...
enum TokenStorage {
    ...
    static var userDID: String? {
        ...
        set {
            if let newValue {
                try? keychain.set(newValue, key: "user_did")
            } else {
                try? keychain.remove("user_did")
            }
        }
    }
    ...
    static var userHandle: String? {
        ...
        set {
            if let newValue {
                try? keychain.set(newValue, key: "user_handle")
            } else {
                try? keychain.remove("user_handle")
            }
        }
    }
    ...
    static var tokenExpiresAt: Date? {
        ...
        set {
            if let newValue {
                try? keychain.set(String(newValue.timeIntervalSince1970), key: "token_expires_at")
            } else {
    ...
    static var userAvatar: String? {
        ...
        set {
            if let newValue {
                try? keychain.set(newValue, key: "user_avatar")
            } else {
                try? keychain.remove("user_avatar")
            }
        }
    }
    ...
    static var grantedScope: String? {
        ...
        set {
            if let newValue {
                try? keychain.set(newValue, key: "token_scope")
            } else {
                try? keychain.remove("token_scope")
            }
        }
    }
