...
struct ReorderablePhotoGrid: View {
    ...
    /// Pre-measured by PhotoEditor from the always-present Group row so the
    /// grid has the correct cellSide from frame zero. Avoids the mid-animation
    /// onGeometryChange correction that was shifting destination positions while
    /// matched-geometry was in-flight and causing jitter.
